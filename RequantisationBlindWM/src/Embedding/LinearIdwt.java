...
public class LinearIdwt {
    ...
    public double[][] LinearIdwt(double[][] coefficient, int[] size, int level, String wavelet_name) {
        
        double[][] reconstructed_image = coefficient;
        FilterBank filter = new FilterBank();
        double[][] qmf = filter.Filter_bank(wavelet_name);
        int nc_x = (int)Math.round(size[1]/ Math.pow(2.0, (double)(level-1)));
        int nc_y = (int)Math.round(size[0]/ Math.pow(2.0, (double)(level-1)));
        ...
        for(int q=0;q<size[1];q++){
            for(int r=0;r<size[0];r++){                
                ...
                if(reconstructed_image[q][r]>255){
                    reconstructed_image[q][r] = 255;
                }             
                if(reconstructed_image[q][r] < 0){
                    reconstructed_image[q][r] = 0;
                }
            }        
        }
